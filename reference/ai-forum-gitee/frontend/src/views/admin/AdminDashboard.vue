<template>
  <div class="admin-dashboard" v-loading="loading">
      ...
      <el-col :span="6">
        <el-card>
          <div class="stat-card">
            ...
            <p class="stat-sub">封禁用户 {{ overview.banned_users || 0 }}</p>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card>
          <div class="stat-card">
            <h3>板块数</h3>
            <p class="stat-value">{{ boardActivity.length }}</p>
            <p class="stat-sub">活跃板块</p>
          </div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="20" class="chart-row">
      <el-col :span="24">
        <el-card>
          <template #header><h3>板块活跃度</h3></template>
...
async function fetchTrendData() {
  try {
    ...
    const dates = rows.map(r => r.stat_date || r.date)
...
async function fetchBoardActivity() {
  try {
    // Board data comes from the daily stats overview
    const rows = overview.value
    boardActivity.value = rows.boards || []
  } catch (e) {
    console.error('Failed to fetch board activity:', e)
  }
