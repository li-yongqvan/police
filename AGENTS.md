...
- **skill 调用要求**：
  ...
  - **`ask-smart-question` 硬性执行规则**：
    - 用户显式调用 `ask-smart-question` 时，默认任务目标是“按模板整理问题并收集缺失信息”，不是诊断原因、修改代码或运行排查命令。
    - 必须先结合本项目上下文，将用户原话匹配到 skill 模板中的字段：问题类型、目标、上下文、期望行为、实际行为、环境、复现步骤、已尝试操作、请求帮助。
    - 对已经明确的信息只做事实提取，不进行未经用户确认的原因判断、技术推断或解决方案建议。
    - 只针对缺失且会影响回答质量的信息提问；在关键字段补齐前，不得自行进入代码分析、浏览器排查、接口测试或文件修改。
    - 只有用户另外明确要求“直接诊断”“分析原因”“提供解决方案”或“修改代码”时，才可以从问题整理模式切换到诊断/实施模式。
    - 输出前必须自检：是否包含“重写后的问题”“为什么更容易回答”“仍缺少的信息”；若用户尚未要求诊断，不得在输出中夹带诊断结论。
- **交付衔接**：
